...
class IRrtStar:
    ...
    def planning(self):
        ...
        for k in range(self.iter_max):
            ...
            if self.Cost(x_nearest) + self.Line(x_nearest, x_rand) + self.Line(x_rand, self.x_goal) < self.budget:  # budget check for nearest parent (to make it more efficient)
                self.V.append(x_new)  # generate an edge to each near point
                x_new.parent = []
                ...
                for x_nearest in self.Near(self.V,x_rand)[:10]:
                    ...
                    if x_new and not self.utils.is_collision(x_nearest, x_new):
                        ...
                        if c_min+self.Line(x_new, self.x_goal) <self.budget: #extra check for budget for actual parent
                            ...
                            x_new.parent.append(x_nearest) #added
    ...
    def Cost(self, node):
        ...
        cost = 0.0
        while node.parent:
            print("node.x: "+str(node.x)+"node.parent[0].x: "+str(node.parent[0].x))
            print("node.y: "+str(node.y)+"node.parent[0].y: "+str(node.parent[0].y))
            cost += math.hypot(node.x - node.parent[0].x, node.y - node.parent[0].y)
            node = node.parent[0]
    ...
    def Info(self,node):
        ...
        while node.parent:
            info += math.hypot(node.x - node.parent[0].x, node.y - node.parent[0].y)
            node = node.parent[0] #fix the [0] part still!!!
        return info
...
def main():
    ...
    rrt_star = IRrtStar(x_start, x_goal, 4, 0.1, 12, 1000)
    rrt_star.planning()

    ...
    def Pruning(self, x_new):
        ...
        for node in self.V:
            if node.x==x_new.x and node.y==x_new.y: #co-located nodes
                ...
                costlist.append(self.Cost(node)) #to compare the costs
                infolist.append(self.Info(node)) #to compare the info values
        # now the pruning
        for node1 in nodelist:
            for node2 in nodelist:
                if self.Cost(node2)<=self.Cost(node1) and self.Info(node2)>self.Info(node1):
                    # prune the node from all nodes and from our lists
                    print("node pruned")
                    self.V.remove(node1)
                    nodelist.remove(node1)
                    costlist.remove(self.Cost(node1))
                    infolist.remove(self.Info(node1))

...
def calc_optimal_path(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=STEP_SIZE):
    ...
    gx = gx - sx
    gy = gy - sy
    paths = calc_all_paths(0, 0, syaw, gx, gy, gyaw, maxc, step_size=step_size)
...
def set_path(paths, lengths, ctypes):
    ...
    #assert path.L >= 0.01 # took it out
    paths.append(path)
...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    # remove unused data
    if len(px)>=1: # i added this/ new
        while px[-1] == 0.0:
            px.pop()
            py.pop()
            pyaw.pop()
            directions.pop()
            if not len(px)>1: # added/ new
                break;
...
def interpolate(ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, directions):
    if m == "S":
        ...
        #print(ind)
        #if len(px)>=ind and ind>-1: # added/ new
        try:
            #print("ind"+str(ind)+" len(px): "+str(len(px)))
            px[ind] = ox + gdx
            py[ind] = oy + gdy
        except:
            print("!!!Something went wrong, px[ind] not possible ")
    ...
    try:
        ...
        print("!!!Something went wrong, pyaw[ind] or directions[ind] not possible ")
